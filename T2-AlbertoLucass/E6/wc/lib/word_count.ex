defmodule WordCount do
  ...
  def read_file(parsed, filename) do
    ...
    Enum.each(flags, fn flag ->
      case flag do
        :lines -> IO.puts("Linhas: #{Enum.count(lines)}")
        :words -> IO.puts("Palavras: #{Enum.count(words)}")
        :chars -> IO.puts("Caracteres: #{Enum.count(chars)}")
        _ -> nil
      end
    end)
  end
...
# mix escript.build
# escript ./word_count ./umtexto.txt -l
# escript ./word_count ./umtexto.txt -w
# escript ./word_count ./umtexto.txt -c
