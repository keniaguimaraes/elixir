# retorna o oposto de um número
anonima = fn a -> -a end

# retorna o quadrado de um número
anonima_abreviada = &(&1 * &1)
...
defmodule FuncaoExemplo do
  ...
  def funcaoDeUmaLinha(),
    do: IO.gets("Digite a frase que quer transformar em maiúscula") |> String.upcase()
  ...
  # dados de uma pessoa estão nesse map, mas só nos interessa a idade
  def pode_viajar_sozinho(%{idade: idade_da_pessoa}) do
    idade_da_pessoa >= 18
  end

  # dados de uma pessoa estão nesse map, mas só nos interessa a idade
  def pode_viajar_sozinho_2(%{idade: idade_da_pessoa} = pessoa) do
    if idade_da_pessoa >= 18 do
      IO.puts("#{pessoa[:nome]} pode viajar sozinho/a, pois tem #{idade_da_pessoa} anos.")
    else
      IO.puts(
        "#{pessoa[:nome]} não pode viajar sozinho/a, pois tem apenas #{idade_da_pessoa} anos."
      )
    end
  end
